...
main() {
    ...
    # Package lists
    arch_packages=(
        "git" "less" "base-devel" "dosfstools" "nano" "rust" "hyprland"
        "waybar" "firefox" "engrampa" "pipewire" "thunar"
        "thunar-archive-plugin" "gvfs" "wireplumber" "ghostty"
        "polkit-gnome" "xdg-desktop-portal-hyprland" "xdg-desktop-portal"
        "pavucontrol" "ttf-font-awesome" "ttf-jetbrains-mono" "qt5-wayland"
        "qt6-wayland" "nwg-look" "papirus-icon-theme" "qt6-svg"
        "qt6-declarative" "cliphist" "wl-clipboard" "xdg-user-dirs" "ly"
    )

    paru_packages=(
        "dracula-gtk-theme" "hyprshot" "swaync" "wlogout" "fuzzel"
        "hyprpaper" "blueberry" "vscodium-bin" "network-manager-applet"
    )

    if [[ -f /etc/arch-release ]]; then
        echo -e "\nWe need all these packages:\n"
        echo -e "Arch packages to install:"
        printf "%-30s %-30s\n" "${arch_packages[@]}" | column -t
        ...
        git clone https://github.com/Broly1/hyprland-dots.git "$temp_dir/hyprland-dots"
        cd "$temp_dir/hyprland-dots" || exit

        directories=(
            "ghostty" "fuzzel" "hypr" "wallpaper" "waybar" "wlogout" "Thunar"
        )

        for dir in "${directories[@]}"; do
            cp -r "$dir" "$HOME/.config/"
            find "$HOME/.config/$dir" -type f -exec chmod +x {} +
            find "$HOME/.config/$dir" -type d -exec chmod +x {} +
        done
